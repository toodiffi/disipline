  ...
  methods: {
    ...
    setDefaultDisableDate() {
      ...
      if (calendarConfig.disableMode && !calendarConfig.disableMode.date) {
        calendarConfig.disableMode.date = dateUtil.toTimeStr(dateUtil.todayFMD());
      }
    ...
    tapDate(e) {
      ...
      for (let plugin of plugins.installed) {
        ...
        if (typeof p.onTapDate === 'function') {
          const { calendarData: __calendarData, calendarConfig: __calendarConfig } = p.onTapDate(
            info,
            calendarData,
            calendarConfig
          );
    ...
    doubleClickJumpToToday() {
      ...
      if (this.lastClick) {
        ...
        if (difference < 500 && this.count >= 2 && typeof this.calendar.jump === 'function') {
