...
import { dateUtil, getCalendarData, setCalendarData, getCalendarConfig } from '../../utils/index';
  ...
  return {
    ...
    beforeRender(calendarData = {}, calendarConfig) {
      ...
      if (selectedDates.length) {
        const selectedDatesStr = selectedDates.map(date => dateUtil.toTimeStr(date));
        _dates.forEach(date => {
    ...
    onTapDate(tapedDate, calendarData = {}, calendarConfig = {}) {
      ...
      const dateIndex = dateUtil.findDateIndexInArray(tapedDate, calendarData.dates);
      ...
      if (!multi) {
        ...
        if (dates[dateIndex].choosed) {
          ...
          calendar.selectedDates = calendar.selectedDates.filter(
            date => dateUtil.toTimeStr(date) !== dateUtil.toTimeStr(dates[dateIndex])
          );
        }
      }
      return {
      ...
      return {
        ...
        setCalendarConfig(config) {
          return new Promise((resolve, reject) => {
            ...
            setCalendarData({ config: conf }, component).then(resolve).catch(reject);
          });
        },
        cancelSelectedDates(cancelDates = []) {
          ...
          if (!cancelDates.length) {
            ...
            const cancelDatesStr = cancelDates.map(date => dateUtil.toTimeStr(date));
        ...
        setSelectedDates: targetDates => {
          ...
          if (!targetDates) {
            ...
            const allSelected = dateUtil.uniqueArrayByDate(selectedDates.concat(targetDates));
            const allSelectedDateStr = allSelected.map(d => dateUtil.toTimeStr(d));
            __dates = dates.map(item => {
              const date = { ...item };
        ...
        setDateStyle: toSetDates => {
          ...
          if (Array.isArray(specialStyleDates)) {
            toSetDates = dateUtil.uniqueArrayByDate([...specialStyleDates, ...toSetDates]);
          }
